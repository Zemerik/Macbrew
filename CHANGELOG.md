...
- **History status toggle**:
  - New `history --no-status` flag to hide exit code indicators (✓/✗) in history output for easier viewing and scripting.
  - Default still shows status symbols, but you can now opt for simpler output.
